      ...
      <div className="flex items-center gap-3 px-4 py-2 bg-white border-b border-gray-100 shrink-0 flex-wrap">
        ...
        {accounts.length > 0 ? (
          <div className="flex items-center gap-2 shrink-0">
            ...
            {sessionError && selectedEmail && (
              <button
                onClick={handleOpenBrowser}
                disabled={openingBrowser}
                className="text-xs px-2 py-1 rounded-md border border-red-200 text-red-500 hover:bg-red-50 disabled:opacity-50 transition-colors shrink-0"
              >
                {openingBrowser ? "Opening…" : "Re-login"}
              </button>
            )}
            {checkingAll ? (
              <button
                onClick={handleStopCheckSessions}
                className="text-xs px-2 py-1 rounded-md border border-orange-300 text-orange-600 hover:bg-orange-50 transition-colors shrink-0"
              >
                {checkProgress
                  ? `Stop (${checkProgress.checked}/${checkProgress.total} — ✓${checkProgress.valid} ⚠${checkProgress.expired})`
                  : "Stop"}
              </button>
            ) : (
              <button
