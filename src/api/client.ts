...
export const api = {
  ...
  aaCheckSessions: () =>
    post<{ message: string; total: number }>("/aa/check-sessions", {}),

  aaCheckSessionsStatus: () =>
    get<{ running: boolean; total: number; checked: number; valid: number; expired: number; errors: number; results: { email: string; check_status: string; last_error: string }[] }>(
      "/aa/check-sessions/status"
    ),
  ...
  aaBatchReloginCancel: () =>
    post<{ cancelled: boolean }>("/aa/batch-relogin/cancel", {}),
